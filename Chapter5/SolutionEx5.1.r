...
corpus.test.char.3.grams <- txt.to.features(corpus.test,
                                       ngram.size=3, features='c')
#
# Select the features:  Here the 200 Most Frequent trigrams
...
#
# Select one query article 
# E.g., the Q55
n <- dim(freqs.train)[1]
aQueryIndex <- 2
aQueryLabel <- rownames(freqs.test)[aQueryIndex]
cat("Euclidan distance with ", aQueryLabel, "\n")
for (anIndex in 1:n) {
  cat (anIndex, rownames(freqs.train)[anIndex], " ")
  cat (euclidian(freqs.test[aQueryIndex,], as.vector(freqs.train[anIndex,])))
  cat("\n")
}
#
# Too much information
#
# STEP 6:  Reduce the output to the minimum
#
# And need to consider all query articles
#
for (aQueryIndex in 1:(dim(freqs.test)[1])) {
   cat("Closest Euclidan distance with", rownames(freqs.test)[aQueryIndex])
   distance <- numeric(n)
   for (anIndex in 1:n) {
     distance[anIndex] <- euclidian(freqs.test[aQueryIndex,],
                                 (freqs.train[anIndex,]))
     }
...
#
# And with the Manhatten distance
#
for (aQueryIndex in 1:(dim(freqs.test)[1])) {
