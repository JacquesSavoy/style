...
plot(mydata[,aVar1], yValue,
     ...
     ylab="Bot vs. Human",
     col=c("blue","red")[mydata$dec], lwd=2,
     pch=c(22, 19)[mydata$dec])
#
# Logistic Regression
#
options(digits=3)
...
#
logReg.mod2 <- glm(dec ~ face, family = binomial, data=mydata)
summary(logReg.mod2)$coef
#
# Logistic regression with two variables mention & face
#
logReg.mod3 <- glm(dec ~ mention+face, family = binomial, data = mydata)
